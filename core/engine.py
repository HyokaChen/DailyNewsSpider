...
MIDDLEWARES = namedtuple("MIDDLEWARES", ['TemplateMiddleware',
                                         'DownloadMiddleware',
                                         'QueueMiddleware'])
...
class Liz2Bird(object):
    ...
    def __init__(self, dir_path, download: Downloader, pipeline: ProcessContent, queue: TaskQueue):
        ...
        self.middlewares = MIDDLEWARES(TemplateMiddleware=TemplateMiddleware(),
    ...
    async def _start(self, is_master=False):
        ...
        if is_master and self.spider_status == SpiderStatus.Null:
            ...
            for template in self.templates:
                ...
                requests, processes, results = self.middlewares.TemplateMiddleware.middle_origin2engine(template)
                ...
                # 放置一些映射关系
                self._queue_push([r.obj2dict() for r in requests], topic=REQUESTS_TOPIC.format(template['SPIDER_NAME']),
                ...
                self._queue_push([r.obj2dict() for r in results], topic=RESULTS_TOPIC.format(template['SPIDER_NAME']),
                ...
                if count == 0:
                    ...
                    if values is not None:
                        self_global_parameters_ = self.middlewares.QueueMiddleware.middle_origin2engine(*[values])
                        self_global_parameters_ = ['{0}:{1}*'.format(global_parameter, template['SPIDER_NAME'])
                    ...
                    try:
                        ...
                        for i, download_content in enumerate(download_content_list):
                            ...
                            if task.next_request is not None:
                                ...
                                next_request_template = self.middlewares.QueueMiddleware.middle_origin2engine(next_request_bytes)[0]
                            ...
                            if task.parallel_request is not None:
                                ...
                                parallel_request_template = \
                                    self.middlewares.QueueMiddleware.middle_origin2engine(parallel_request_bytes)[0]
    ...
    def stop(self):
        ...
        for topic in topics:
            ...
            if count == 0:
                ...
                if values is not None:
                    global_parameters = self.middlewares.QueueMiddleware.middle_origin2engine(*[values])
    ...
    def _queue_push(self, task, topic=None, way=WayType.LPUSH, key_ids=None):
        ...
        if topic is None:
            for ta in task:
                ...
                new_ta = self.middlewares.QueueMiddleware.middle_engine2origin(*[ta])
                self.queue.push(topic, new_ta, way=way, key_ids=key_ids)
        else:
            task = self.middlewares.QueueMiddleware.middle_engine2origin(*task)
    ...
    def _task_pop(self, count=1):
        ...
        if task is None:
            return None
        return total_task, [Task(**t) for t in self.middlewares.QueueMiddleware.middle_origin2engine(*task)]
    ...
    async def _download(self, task):
        ...
        :return: 下载结果
        """
        task = self.middlewares.DownloadMiddleware.middle_engine2origin(task)
        old_content = await self.download.single_download(task)
        return self.middlewares.DownloadMiddleware.middle_origin2engine(old_content)
    ...
    def _process(self, content, task: Task):
        ...
        if task.process_id is not None and process_bytes is None:
            ...
            process_template = self.middlewares.QueueMiddleware.middle_origin2engine(process_bytes)[0]
        ...
        result_template = self.middlewares.QueueMiddleware.middle_origin2engine(result_bytes)[0]
        ...
        try:
            ...
            if not is_stopped and task.request['is_multiple']:
                ...
                current_request_template = self.middlewares.QueueMiddleware.middle_origin2engine(current_request_bytes)[0]
        ...
        try:
            ...
            if DOT in expression and 'http' not in expression:
                ...
                if left == PROCESSES:
                    ...
                    process_template = self.middlewares.QueueMiddleware.middle_origin2engine(process)[0]
                ...
                # 包含result处理
                    ...
                    if len(parameter_name) == 0:
                        result = self.queue.find(RESULTS_TOPIC.format(parameters['spider_name']),
                                                 way=WayType.HGET, key=number)
                        result_template = self.middlewares.QueueMiddleware.middle_origin2engine(result)[0]
